...
class MenuScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let menuBgNode = SKSpriteNode(imageNamed: "menu_bg")
        menuBgNode.size = CGSize(width: 72/47*size.height, height: size.height)
        menuBgNode.position = CGPoint(x: size.width / 2, y: size.height / 2)
        menuBgNode.anchorPoint = CGPoint(x: 0.5, y: 0.5)
        menuBgNode.zPosition = 0
        menuBgNode.name = "menu_bg"
        addChild(menuBgNode)
